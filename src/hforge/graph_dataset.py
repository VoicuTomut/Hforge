...
class LazyGraphDataset(Dataset):
    def __init__(self, parent_dir=None, n_atoms_dir=None, max_samples=None, seed=42):
        ...
        self.files = []
        if n_atoms_dir is not None and parent_dir is None:
            self.files.append(sorted([os.path.join(n_atoms_dir, f) for f in os.listdir(n_atoms_dir) if f.endswith(".pt")]))
            self.files = self.flatten(self.files)

        elif n_atoms_dir is None and parent_dir is not None:
            ...
            raise ValueError("Either parent_dir or n_atoms_dir must be provided but not both.")
