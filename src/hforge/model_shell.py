...
class ModelShell(torch.nn.Module):
    ...
    def __init__(self, config_routine):
        ...
        self.node_extraction=None
        self.edge_extraction=None
...
! NodeExtractionBasic
"""
class NodeExtractionBasic(torch.nn.Module):
    def __init__(self, config_routine):
        super(NodeExtractionBasic, self).__init__()

        # get the input output representations for etch step:
        orbitals=config_routine["orbitals"]

        atoms=orbitals.keys()

        # TODO: add this part
        self.custom_evolution=None


        self.extraction_heads={}
        for atom in atoms:
            key=[atom,atom]
            self.extraction_heads[atom]=None
    ...
    def forward(self, graph_data, embeddings, atomic_env_descriptor):
        ...
        # Additional interaction just for onsite prediction.
        # TODO: add this part
        if self.custom_evolution is not  None:
            evolve_nodes , evolved_edges=self.custom_evolution(graph_data, embeddings, atomic_env_descriptor)


        # Iterate over etch node
        # Todo try to paralelize:
        on_sites=[]
        for i, atom in enumerate():

            on_site=self.extraction_heads[atom](env1)
            on_sites.append(on_site)
...
class EdgeExtractionBasic(torch.nn.Module):
    def __init__(self, config_routine):
    ...
    def forward(self, graph_data, embeddings, atomic_env_descriptor):
        ...
        of_sites = []
        for i, atom in enumerate():
            of_site = self.extraction_heads[atom]()
            of_sites.append(of_site)
